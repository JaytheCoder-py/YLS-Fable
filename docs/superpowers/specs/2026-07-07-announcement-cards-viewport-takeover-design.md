...
| Where does the extra height come from? | `.feature-section` becomes a `min-height: 100svh` flex column with the pair vertically centered. Height is reserved up front → zero reflow below while the cards grow (no ScrollTrigger position drift). |
...
- `start: 'top 20%'` — fires ~200px (two wheel notches) later than the old `'top 40%'`.
  Because the section top is unchanged in the document (the reservation grows downward),
  this is a true 0.2×viewport delay in scroll distance.
...
- Scrub distance = 20% of the viewport (~200px): a deliberately punchy takeover.
